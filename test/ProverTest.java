...
public class ProverTest {
	...
	public void testGameRulesetTest() throws IOException, URISyntaxException, GDLSyntaxException {
		...
		/*
		assertThat(initState.evaluate(GdlParser.parseString("(role p2)").getChild(0)), is(true));
		assertThat(initState.evaluate(GdlParser.parseString("(true (step 1))").getChild(0)), is(true));
		assertThat(initState.evaluate(GdlParser.parseString("(legal p2 (move 1))").getChild(0)), is(true));
		assertThat(initState.evaluate(GdlParser.parseString("(<= (legal p2 (move 1)) (role p2))").getChild(0)),
				is(true));
	...
	public void meierRulesetTest() throws GDLSyntaxException, IOException, URISyntaxException {
		...
		for (String head : prover.getRuleSet().keySet()) {
			...
			if (head.length() >= better_values.length() && head.substring(0, better_values.length()).equals(better_values)) {
			...
			for (Set<String> disjunct : prover.getRuleSet().get(head)){
				for (String literal : disjunct) {
					if (literal.length() >= distinct.length() && literal.substring(0, distinct.length()).equals(distinct)) {
